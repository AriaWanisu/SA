...
func (sq *SubjectQuery) QueryOwner() *TeacherQuery {
...
func (sq *SubjectQuery) QueryCourseItems() *CourseItemQuery {
	query := &CourseItemQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) WithOwner(opts ...func(*TeacherQuery)) *SubjectQuery {
	query := &TeacherQuery{config: sq.config}
...
func (sq *SubjectQuery) WithCourseItems(opts ...func(*CourseItemQuery)) *SubjectQuery {
	query := &CourseItemQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withCourseItems = query
	return sq
}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	...
	if query := sq.withCourseItems; query != nil {
		...
		query.Where(predicate.CourseItem(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.CourseItemsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
