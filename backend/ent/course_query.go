...
func (cq *CourseQuery) QueryOwner() *TeacherQuery {
...
func (cq *CourseQuery) QueryCourseItems() *CourseItemQuery {
	query := &CourseItemQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CourseQuery) WithOwner(opts ...func(*TeacherQuery)) *CourseQuery {
	query := &TeacherQuery{config: cq.config}
...
func (cq *CourseQuery) WithCourseItems(opts ...func(*CourseItemQuery)) *CourseQuery {
	query := &CourseItemQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCourseItems = query
	return cq
}
...
func (cq *CourseQuery) sqlAll(ctx context.Context) ([]*Course, error) {
	...
	if query := cq.withCourseItems; query != nil {
		...
		query.Where(predicate.CourseItem(func(s *sql.Selector) {
			s.Where(sql.InValues(course.CourseItemsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.course_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "course_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
