...
async function checkExists(destination) {
  await tasks.Task.forItem("checking", destination, async () => {
    if (await fs.pathExists(destination)) {
      const existing = chalk2.cyan(destination.replace(`${index.paths.targetRoot}/`, ""));
...
async function addPluginToApp(rootDir, pluginName, pluginPackage) {
...
var createPlugin = async (cmd) => {
  ...
  const name = cmd.scope ? `@${cmd.scope.replace(/^@/, "")}/plugin-${answers.id}` : `plugin-${answers.id}`;
  const npmRegistry = cmd.npmRegistry && cmd.scope ? cmd.npmRegistry : "";
  const privatePackage = cmd.private === false ? false : true;
  const isMonoRepo = await fs.pathExists(index.paths.resolveTargetRoot("lerna.json"));
  ...
  const pluginDir = isMonoRepo ? index.paths.resolveTargetRoot("plugins", answers.id) : index.paths.resolveTargetRoot(answers.id);
  const ownerIds = tasks.parseOwnerIds(answers.owner);
  const {version: version2} = isMonoRepo ? await fs.readJson(index.paths.resolveTargetRoot("lerna.json")) : {version: "0.1.0"};
  tasks.Task.log();
  ...
  try {
    ...
    if (await fs.pathExists(appPackage) && !cmd.backend) {
      ...
      await addPluginDependencyToApp(index.paths.targetRoot, name, version2);
      tasks.Task.section("Import plugin in app");
      await addPluginToApp(index.paths.targetRoot, answers.id, name);
    }
    if (ownerIds && ownerIds.length) {
      await tasks.addCodeownersEntry(codeownersPath, `/plugins/${answers.id}`, ownerIds);
    }
    tasks.Task.log();
    tasks.Task.log(`🥇  Successfully created ${chalk2.cyan(`${name}`)}`);
    tasks.Task.log();
    tasks.Task.exit();
  } catch (error) {
...
//# sourceMappingURL=createPlugin-aa3472ed.js.map
